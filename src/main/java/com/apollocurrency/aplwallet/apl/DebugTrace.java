...
public final class DebugTrace {
    ...
    static final boolean LOG_UNCONFIRMED = Apl.getBooleanProperty("apl.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = Apl.getStringListProperty("apl.debugTraceAccounts");
        String logName = Apl.getStringProperty("apl.debugTraceLog");
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        Apl.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        Apl.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        Apl.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void trace(Block block) {
        for (Transaction transaction : block.getTransactions()) {
            ...
            if (transaction.getAmountATM() > 0 && recipientId == 0) {
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("height", String.valueOf(Apl.getBlockchain().getHeight()));
    ...
    private void crowdfunding(Currency currency) {
        ...
        if (!currency.is(CurrencyType.CLAIMABLE)) {
            map.put("currency cost", String.valueOf(Math.multiplyExact(currency.getReserveSupply(), currency.getCurrentReservePerUnitATM())));
        }
    ...
    private Map<String,String> getValues(long accountId, Account.AccountAsset accountAsset, boolean unconfirmed) {
        ...
        if (unconfirmed) {
            map.put("unconfirmed asset balance", String.valueOf(accountAsset.getUnconfirmedQuantityATU()));
        } else {
    ...
    private Map<String,String> getValues(long accountId, Transaction transaction, Attachment attachment, boolean isRecipient) {
        ...
        if (attachment instanceof Attachment.ColoredCoinsOrderPlacement) {
            ...
            map.put("sell rate", String.valueOf(publishOffer.getSellRateATM()));
            ...
            BigInteger buyCost = BigInteger.valueOf(publishOffer.getBuyRateATM()).multiply(BigInteger.valueOf(buyUnits));
