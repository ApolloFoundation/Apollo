...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Apl.getBlockchainProcessor().isScanning() ?
                    Math.max(Apl.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
    ...
    private boolean doesNotExceed(int height) {
        return Apl.getBlockchain().getHeight() <= height && ! (isPersistent() && Apl.getBlockchainProcessor().isScanning());
    }
