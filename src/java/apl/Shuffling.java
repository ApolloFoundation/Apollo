...
import apl.crypto.AnonymouslyEncryptedData;
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (holdingType != HoldingType.APL) {
                    participantAccount.addToBalanceNQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_ATM);
                }
            }
        }
        for (byte[] recipientPublicKey : recipientPublicKeys) {
            ...
            if (holdingType != HoldingType.APL) {
                recipientAccount.addToBalanceAndUnconfirmedBalanceNQT(event, this.id, Constants.SHUFFLING_DEPOSIT_ATM);
            }
        }
        ...
        try (DbIterator<ShufflingParticipant> participants = ShufflingParticipant.getParticipants(id)) {
            for (ShufflingParticipant participant : participants) {
                ...
                if (participantAccount.getId() != blamedAccountId) {
                    if (holdingType != HoldingType.APL) {
                        participantAccount.addToUnconfirmedBalanceNQT(event, this.id, Constants.SHUFFLING_DEPOSIT_ATM);
                    }
                } else {
                    if (holdingType == HoldingType.APL) {
                        participantAccount.addToUnconfirmedBalanceNQT(event, this.id, -Constants.SHUFFLING_DEPOSIT_ATM);
