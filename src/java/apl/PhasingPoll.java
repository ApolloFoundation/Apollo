...
public final class PhasingPoll extends AbstractPoll {
    ...
    static DbIterator<TransactionImpl> getFinishingTransactions(int height) {
        ...
        try {
            ...
            return BlockchainImpl.getInstance().getTransactions(con, pstmt, false);
        } catch (SQLException e) {
            DbUtils.close(con);
            throw new RuntimeException(e.toString(), e);
        }
    ...
    public long countVotes() {
        ...
        int height = Math.min(this.finishHeight, Apl.getBlockchain().getHeight());
