...
class CurrencyBuyOfferTableTest {
    ...
    @Test
    void testLoad() {
        CurrencyBuyOffer asset = table.get(table.getDbKeyFactory().newKey(td.OFFER_0));
        assertNotNull(asset);
        assertEquals(td.OFFER_0, asset);
    }

    @Test
    void testLoad_returnNull_ifNotExist() {
        CurrencyBuyOffer asset = table.get(table.getDbKeyFactory().newKey(td.OFFER_NEW));
        assertNull(asset);
    }
