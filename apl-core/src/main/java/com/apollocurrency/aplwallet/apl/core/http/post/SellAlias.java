...
public final class SellAlias extends CreateTransaction {
    ...
    public JSONStreamAware processRequest(HttpServletRequest req) throws AplException {
        ...
        Account owner = HttpParameterParser.getSenderAccount(req);

        long priceATM =
                HttpParameterParser.getLong(req, "priceATM", 0L, CDI.current().select(BlockchainConfig.class).get().getCurrentConfig().getMaxBalanceATM(),
                true);
