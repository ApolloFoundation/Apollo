...
@Slf4j
@Singleton
public class CurrencyBuyOfferServiceImpl {
...
//            dbClause = dbClause.and(availableOnlyDbClause);
    ...
    public CurrencyBuyOffer getOffer(final long currencyId, final long accountId) {
        return currencyBuyOfferTable.getBy(new DbClause.LongClause("currency_id", currencyId).and(new DbClause.LongClause("account_id", accountId)));
    }
    ...
    public void remove(CurrencyBuyOffer buyOffer) {
        ...
        buyOffer.setHeight(height); // important to set that !
