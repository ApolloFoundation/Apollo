...
public interface TransactionIndexDao {
    ...
    @Transactional
    @SqlUpdate("UPDATE transaction_shard_index SET block_id =:blockId where transaction_id =:transactionId AND block_id =:blockId")
    void updateBlockIndex(@BindBean TransactionIndex transactionIndex);

    @Transactional
    @SqlUpdate("DELETE FROM transaction_shard_index where transaction_id =:transactionId AND block_id =:blockId")
    void hardDeleteTransactionIndex(@BindBean TransactionIndex transactionIndex);

    @Transactional
    @SqlUpdate("DELETE FROM transaction_shard_index")
    void hardDeleteAllTransactionIndex();
