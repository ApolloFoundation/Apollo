...
public interface TransactionIndexDao {
    ...
    @Transactional(readOnly = true)
    @SqlQuery("SELECT count(*) FROM transaction_shard_index where height < COALESCE((select shard_height from shard where shard_id =:shardId),0) AND height >= IFNULL((select shard_height from shard where shard_height < (select shard_height from shard where shard_id =:shardId) ORDER BY height desc LIMIT 1),0)")
