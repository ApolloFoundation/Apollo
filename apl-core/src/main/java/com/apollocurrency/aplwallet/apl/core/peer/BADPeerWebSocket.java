 ...
 */
@WebSocket
public class BADPeerWebSocket {
    private static final Logger LOG = getLogger(BADPeerWebSocket.class);
    ...
    static {
        try {
            ...
            LOG.error("Unable to start WebSocket client", exc);
            peerClient = null;
        }
    }
    @Getter
     ...
     */
    public BADPeerWebSocket(Peer p) {
        clientPeer = p;
        peerServlet = null;
    }
     ...
     */
    public BADPeerWebSocket(PeerServlet peerServlet, Peer peer) {
    ...
    public boolean startClient(URI uri, Peer p) throws IOException {
        ...
        // since the ARS server will issue multiple concurrent requests to the same peer.
        ...
        try {
            ...
            if (! peerClient.isStarted()) {
                LOG.debug("WebSocket client not started or shutting down");
                throw exc;
            }
            LOG.debug(String.format("WebSocket connection to %s failed", address), exc);
        } catch (Exception exc) {
