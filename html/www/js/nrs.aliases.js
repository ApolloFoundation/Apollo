...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.setup.aliases = function () {
        var options = {
            "id": 'sidebar_aliases',
            "titleHTML": '<i class="mdi mdi-bookmark"></i><span class="hide-menu"> Aliaces </span>',
            "page": 'aliases',
            "desiredPosition": 100,
    ...
    NRS.forms.sellAlias = function ($modal) {
        ...
        if (data.modal == "cancel_alias_sale") {
            ...
            if (data.recipient == NRS.constants.GENESIS_RS) {
                if (!data.priceAPL || data.priceAPL == "0") {
                    return {
                        "error": $.t("error_not_specified", {
    ...
    NRS.forms.buyAliasError = function () {
        $("#buy_alias_modal").find("input[name=priceAPL]").prop("readonly", false);
    };
        ...
        if (alias) {
            ...
            NRS.sendRequest("getAlias", {
                ...
                if (response.errorCode) {
                    ...
                    } else if ((aliasURI = /acct:(.*)@apl/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
    ...
    NRS.forms.setAlias = function ($modal) {
        ...
        if (data.type == "account") {
            if (!(/acct:(.*)@apl/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
                if (NRS.isRsAccount(data.aliasURI)) {
                    ...
                    if (!address.set(data.aliasURI)) {
                        ...
                        data.aliasURI = "acct:" + data.aliasURI + "@apl";
                    }
    ...
    function setAliasType(type, uri) {
        ...
        if (type == "uri") {
            ...
            if (uri) {
                var match = uri.match(/acct:(.*)@apl/i);
                if (!match) {
