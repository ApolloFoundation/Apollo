...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.listAccounts = function() {
        ...
        loginAccount.empty();
		if (NRS.getStrItem("savedAplAccounts") && NRS.getStrItem("savedAplAccounts") != ""){
			$('#login_account_container').show();
			$('#login_account_container_other').hide();
			var accounts = NRS.getStrItem("savedAplAccounts").split(";");
	...
	NRS.removeAccount = function(account) {
		var accounts = NRS.getStrItem("savedAplAccounts").replace(account+';','');
		if (accounts == '') {
			NRS.removeItem('savedAplAccounts');
		} else {
			NRS.setStrItem("savedAplAccounts", accounts);
		}
		NRS.listAccounts();
	};

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedAplAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedAplAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedAplAccounts", accountsStr + account + ";");
    }
			...
			NRS.sendRequest(accountRequest, requestVariable, function(response, data) {
				...
				NRS.sendRequest("getAccountPublicKey", {
					...
					if (NRS.getStrItem("savedAplAccounts") && NRS.getStrItem("savedAplAccounts")!=""){
						...
						accounts = NRS.getStrItem("savedAplAccounts").split(";");
						$.each(accounts, function(index, account) {
							if (account != ''){
	...
	NRS.initPluginWarning = function() {
		...
		if (NRS.activePlugins) {
			...
			html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Apl passphrase.";
			html += "</p>";
